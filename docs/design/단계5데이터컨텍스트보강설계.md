...
| **하위 과제** | **우리 서비스에서** | **판정** |
| --- | --- | --- |
| **1. 텍스트 RAG** | ③ 대화 추천의 근거 생성(`reason_short`+`reason_long`+`match_basis`, 한 호출) · ③ spec 델타 추출 | **핵심 해당** — §4-A |
...
| ④ `POST /recommendations/feed` | 취향 centroid 유사도 기반 retrieval 채점. **passage 증강·이유 문구 없음** — 즉시성 요구(단계 4 파이프라인 C) | **변경 없음** |
