...
> **⚠️ 2026-09-21 결정 변경 — 오케스트레이션 프레임워크 도입**
>
> 아래 "프레임워크: LangChain을 V1에 도입하지 않는 결정"은 팀 논의 끝에 **뒤집혔다.** **최종안**: ③ 대화 추천과 ⑤ 취향 기억 추출은 **LangChain**, ⑦ 쇼핑 에이전트는 **LangGraph**로 구현한다. 추상화 뒤에서 디버깅·장애 추적이 어려워지는 대가는 감수하기로 했다. 파이프라인의 단계 구성과 역할 경계(결정론적 코드가 후보·점수를 만들고 LLM은 그 안에서만 고른다)는 이 결정으로 바꾸지 않는다.
>
> 아래 본문의 "미채택" 판정과 "재평가 트리거"는 **번복된 설계를 이력 추적용으로 남긴 것**이며 현재 구현 기준이 아니다. 2026-09-21 시점 코드(`app/gateway/llm.py`)는 아직 `openai` SDK 직접 호출이고, LangChain 전환은 별도 후속 작업이다.
...
```mermaid
flowchart TB
    EMB["② /embeddings — bge-m3-2026q3 (dim 1024)<br/>모든 벡터의 단일 좌표계"]
    V[("커머스 복제 테이블 · AI Postgres 안<br/>v_books · v_user_purchases · v_user_library<br/>v_user_reviews · v_book_popularity<br/>(원본은 BE MySQL · 단방향 복제 · 역방향 없음)")]

    subgraph UP["상류 — 취향 형성 (파이프라인 D)"]
        P5["⑤ /preferences/extractions (V2)<br/>야간배치: 대화→취향사실 LLM추출→임베딩"]
        P6["⑥ /preferences/profile (V1)<br/>온보딩·기억변경 트리거<br/>벡터 가중평균 집계 (LLM·임베딩 재호출 없음)"]
        P5 -.->|memories with vector| P6
        P6 --> PROF[("user_profiles<br/>centroid + tag_weights + computed_at<br/>개인화 단일 소스")]
    end

    subgraph LLMFREE["LLM 비의존 (LLM 장애에도 동작)"]
        P1["① /search (V1)<br/>BM25/pg_trgm + 벡터 + RRF + 규칙<br/>0건 → fallback 배너"]
        P4["④ /recommendations/feed (V1)<br/>규칙신호 + centroid 유사도 가중합<br/>목록 비저장, 커서만"]
    end

    subgraph LLMDEP["LLM 의존"]
        P3["③ /recommendations/chat<br/>V1 텍스트 / V2 이미지<br/>LLM = spec 델타 + 10 후보 중 3권 선택 + 이유 2종(short+long)<br/>후보 풀·점수는 결정론 / 최종 3권·순위는 LLM"]
        P7["⑦ /agent/act (V2)<br/>지시어해소 → 의도분류 → tool루프(≤3) → grounding"]
    end
    ...
    P1 -.->|"0건 배너 → 사용자 클릭"| P3
    P3 -->|"cards + reason_long"| DET["도서 상세 페이지<br/>BE가 보관한 긴 이유 그대로 표시<br/>(AI 재호출 없음)"]
    P3 -.->|context_cards| P7
    P7 -->|"cart/order/inventory tool"| BE["BE 커머스<br/>(V1엔 없는 유일한 역방향 호출.<br/>DB 복제는 BE→AI 단방향뿐)"]
```
...
flowchart LR
    ...
    A --> C["② /embeddings 호출<br/>bge-m3-2026q3, purpose:query"]
    C --> D[pgvector ANN 검색]
    B --> E[RRF 결합]
    D --> E
    E --> F["규칙 스코어링<br/>sort=popular면 v_book_popularity 반영"]
    F --> G{0건?}
    G -- Yes --> H["results: [], fallback 배너 신호<br/>'AI 추천에게 물어볼까요?'"]
...
## 파이프라인 B — 대화형 추천 (`POST /recommendations/chat`, 
V1 텍스트 턴 / V2 이미지 턴)

```mermaid
flowchart TD
    A[발화 + 이전 spec + exclude_book_ids] --> S["요청 spec 6키 스키마 검증 (LLM 호출 전)<br/>위반 시 422 spec_schema_violation<br/>→ 호출자가 초기 spec으로 1회 재시도"]
    S --> B["LLM 호출 1: spec 델타 추출<br/>6키 고정 스키마"]
    B --> C[서버가 기존 spec에 델타 병합]
    C --> D["하이브리드 후보 10권<br/>(①과 같은 BM25+벡터+RRF 재사용,<br/>spec.semantic/filters 적용)"]
    D --> E["결합 스코어링<br/>질의 유사도 + 취향 유사도<br/>(centroid + computed_at 이후 이력 가산)"]
    E --> F["후보에서 제외:<br/>구매한 책 · 별점 1~2 책 · exclude"]
    F --> G{후보 있음?}
    G -- Yes --> H["LLM 호출 2 (1회):<br/>10 후보 중 3권 선택 + 그 3권에<br/>reason_short + reason_long + match_basis"]
    H --> I["고른 순서 = rank · match_score는 후보 점수 그대로(rank 순 ≠ 내림차순)<br/>reason_short 못 만든 카드는 뺌 (3장 미만 가능)<br/>reason_long만 비면 카드 남기고 reason_long:null"]
    I --> J["응답: cards + reply + followup:null<br/>+ buttons:[] + degraded:false"]
    G -- No/여전히 모호 --> K["cards:[], followup에 되묻는 문장"]
```

```mermaid
flowchart LR
    A2["(V2) image_ref 도착<br/>BE presigned URL"] --> B2[표지 인식]
    B2 --> C2{단일 확실 매칭?}
    C2 -- Yes --> D2["recognition.recognized:true + book_id<br/>spec.anchor_book을 그 book_id로 갱신<br/>유사 도서 카드 + library_add 버튼"]
    C2 -- 여럿 --> E2["recognition.candidates[] 최대3 (confidence 내림차순)<br/>spec.anchor_book은 아직 null 유지<br/>confirm_book/retake 버튼"]
...
```mermaid
flowchart LR
    A["⑥ 취향 프로필 조회<br/>centroid + tag_weights + computed_at"] --> B["규칙 신호<br/>작가·카테고리·태그·이력·인기"]
    A --> C[centroid 코사인 유사도]
    VW[("v_user_* (이력) · v_book_popularity (인기)<br/>AI Postgres 복제 테이블")] -.->|computed_at 이후 이력 가산| B
    B --> D[가중합 = match_score]
    C --> D
    D --> E{cold_start?}
...
flowchart TD
    subgraph NIGHTLY["⑤ /preferences/extractions — 야간 배치 (V2)"]
        A["활동 있던(종료된) 대화 세션<br/>세션 단위 호출"] --> B["LLM: 세션 전체에서 취향 사실 추출<br/>type/value, confidence 0.5 미만은 반환 안 함"]
        B --> C["각 사실 임베딩<br/>bge-m3-2026q3"]
        C --> D["BE가 취향 테이블에 한 행씩 저장<br/>(source_conversation_id로 재처리 시 교체)"]
    end
    subgraph REBUILD["⑥ /preferences/profile — 재계산 (V1, 동기)"]
        E["트리거: 온보딩 완료 직후 · 취향 기억 변경 시<br/>(구매·리뷰가 늘었다고 부르지 않음)"] --> F["입력: onboarding + memories[]<br/>(memories는 이미 vector 포함)"]
        F --> G["복제 테이블 조회: v_user_purchases ∪ v_user_library ∪ v_user_reviews<br/>(user_id로. 요청 본문에 없음)"]
        G --> H["가중평균 centroid<br/>좋아한책 ∪ 작가벡터 ∪ 카테고리·태그 사전벡터<br/>∪ 기억벡터 ∪ 이력 책벡터(구매3 / 리뷰4~5점2 / 담기1)"]
        H --> I["태그 가중치 집계<br/>(리뷰1~2점 −2: centroid엔 빼고 카테고리 점수만 깎음)"]
        I --> J["user_profiles upsert<br/>computed_at = 반영한 이력 행들의 최대 시각 (계산 시각 아님)<br/>(user_id 단위 직렬, last-writer-wins)<br/>순위 영향값 바뀔 때만 profile_version +1"]
    end
    D -.->|다음 재계산 트리거 시 memories로 전달| E
    J --> K["파이프라인 B·C가 참조하는<br/>개인화 단일 소스"]
```
...
```mermaid
flowchart TD
    A[발화 + context_cards + focused_book_id] --> B["지시어 해소<br/>'N번' → context_cards[N-1], '이거' → focused_book_id"]
    B --> C{"confidence 0.6 이상?"}
    C -- No --> D["되물음 발화, tool 미실행 (200)"]
    C -- Yes --> E["의도 분류<br/>담기·수량변경·장바구니조회·주문요약·재고·골라담기·비교"]
    E --> F["tool 루프 (상한 3스텝, 독립 호출 병렬)<br/>골라담기: recommendations.candidates(내부)<br/>→ inventory.check/book.detail로 가격·재고 재검증<br/>→ 조합 전수열거로 결정적 선택"]
    F --> G{grounding 검증}
...
| 벡터 저장소 | AI 전용 PostgreSQL + pgvector (HNSW) | BE가 MySQL이라 DB 공유 불가(구 "Postgres 공유" 안 무효). 

도서 임베딩·취향 프로필·멱등 기록을 한 Postgres에 둠 → 전용 벡터DB 불필요 | AI 쪽 데이터스토어 1개. 

대신 BE→AI 단방향 복제 파이프라인이 필요 |
| 커머스 데이터 접근 | BE MySQL → AI Postgres **단방향 복제된 실제 테이블**

AI엔 BE MySQL 연결 없음 | 요청 본문으로 실어 나르기엔 크고, 원본은 어차피 BE가 소유. 

컬럼 집합·허용 지연·스키마 변경 통보를 계약면으로 두면 BE 내부 스키마 변경과 무관 | 이력·인기 값의 단일 출처 → 검색 인기순 = 추천 인기 항. 

허용 지연은 신선도 예산 표(가격·재고 5분 / 신간+임베딩 1h / 인기 24h / 이력 5분). 

행 없으면 해당 항 0점 200, AI Postgres 응답 불능은 500 |
| 대화·이유 생성 LLM | 소형 텍스트 LLM, 상용 API 우선 | 스트리밍 필요(③), 초기 구축비용 낮음. 

호출 지점이 코드에 2곳(③ 델타·③ 이유)뿐이라 교체 용이 | 빠른 출시. 트래픽 증가 시 자체 서빙 손익분기 재평가 (단계 6) |
| 오케스트레이션 프레임워크 | ~~미채택 (V1 전체)~~ → **③⑤ LangChain · ⑦ LangGraph 채택** (2026-09-21 팀 논의로 변경) | 이전 판단: ③은 고정 선형 체인(LLM 2회 + 결정론적 검색·채점)

 ⑦도 최대 3스텝 고정 tool 세트 | ~~의존성 최소화. **⑦의 tool loop이 복잡해지면 재평가**~~ 추상화 뒤 디버깅 난이도는 감수 |
| 표지 인식기 | 생성형 VLM 미채택

이미지 인코더(자체) + OCR + RRF 최근접 검색 | "사진 → book_id" 단일 인식에 생성 모델 불필요. 

검색 파이프라인의 RRF를 그대로 재사용 | 단계 5에서 확정. 구체 인코더 모델은 실측 후 선정 |

### 프레임워크: LangChain을 V1에 도입하지 않는 결정

> **번복 공지 (2026-09-21)** — 팀 논의로 도입하기로 바꿨다. ③·⑤는 LangChain, ⑦은 LangGraph. 아래 판정 표와 재평가 트리거는 미채택 당시의 판단이며 이력 추적용으로 그대로 둔다.

과제 1번 예시는 "LangChain으로 질의·검색·응답을 체인으로 구성"을 전제한다. ~~**V1에서 도입하지 않는다.**~~

| 파이프라인 | 구조 | 프레임워크가 해결할 문제가 있나 | 판정 |
| --- | --- | --- | --- |
| ③ 대화 추천 | 델타추출(LLM) → 하이브리드검색 → 결합스코어링 → 3권 선택+이유생성(LLM 1회) → 카드 조립. 

**고정 선형**, 분기는 "후보 0 → followup" 하나 | 체인이 정적이라 그래프 오케스트레이션 이득 없음. 

프롬프트·파서·재시도는 얇은 함수로 충분 | ~~미채택~~ **LangChain 채택** |
...
- **재평가 트리거 (⑦ 한정)** — 도입이 확정돼(2026-09-21) 더는 판정에 쓰지 않는다. 미채택 당시 기준을 이력으로 남긴다.
    ...
    - **기대 효과 (미채택 당시 — 2026-09-21 도입 결정으로 더는 유효하지 않음)**: V1 의존성 트리를 얇게 유지(FastAPI + 임베딩 SDK + LLM SDK)
