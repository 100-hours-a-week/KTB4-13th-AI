...
| 오케스트레이션 프레임워크 | 미채택 (V1 전체) | ③은 고정 선형 체인(LLM 2회 + 결정론적 검색·채점)

 ⑦도 최대 3스텝 고정 tool 세트 | 의존성 최소화. **⑦의 tool loop이 복잡해지면 재평가** |
| 표지 인식기 | 생성형 VLM 미채택
...
프롬프트·파서·재시도는 얇은 함수로 충분 | 미채택 |
| ①④·⑥ | LLM이 없거나(①④), 순수 집계(⑥) | 애초에 "체인"이 아님 | 해당 없음 |
| ⑤ 야간 추출 (V2) | 세션당 LLM 1회 + 임베딩 1회 배치 | 선형 2스텝. 배치 스케줄링은 프레임워크 밖 문제 | 미채택 |
| ⑦ 에이전트 (V2) | 지시어해소 → 의도분류 → **tool 루프(≤3스텝)** → grounding → 조립 | 반복 tool 호출·상태 누적·중단조건·부분실패 | **미채택하되 재평가 트리거 명시** |
- **재평가 트리거 (⑦ 한정)**
...
- **쇼핑 에이전트**
    ...
    - 여기서 프레임워크(LangGraph류) 도입 여부는 스텝 수·tool 동적성이 늘어날 때 재평가 대상.
- **비전 다단계 파이프라인은 불필요**
...
- ⑦ tool loop이 향후 스텝 수·동적 tool 선택으로 복잡해질 경우 프레임워크(LangGraph 등) 도입 재검토
