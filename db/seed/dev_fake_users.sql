...
--
-- book_id 는 900001~900005, user_id 는 9001~9004 대역을 써서 나중에 실제
-- 카탈로그·BE 유저 ID와 절대 안 겹치게 한다.
--
-- 유저 4명의 역할 (왜 이렇게 나눴는지):
--   9001 (A) 이력 전혀 없음            → cold_start: true 가 실제로 나오는지
--   9002 (B) 구매+선호리뷰+담기 각 1권 → 여러 신호(+3,+2,+1)가 합산되는지
--   9003 (C) 비선호 리뷰(2.0) 1권      → 그 책이 후보에서 실제로 빠지는지
...
VALUES
    (900001, '[TEST] 비 오는 날의 산책', '테스트작가1', '테스트출판사', 13500, true, '', '에세이', 2023, '비 오는 날 읽기 좋은 잔잔한 에세이.'),
    (900002, '[TEST] 이별 후의 위로', '테스트작가2', '테스트출판사', 12800, true, '', '에세이', 2022, '이별 후 마음을 다독이는 짧은 소설.'),
    (900003, '[TEST] 성장의 기록', '테스트작가3', '테스트출판사', 14000, true, '', '한국소설', 2024, '한 사람의 성장을 담은 잔잔한 이야기.'),
    (900004, '[TEST] 힐링 에세이', '테스트작가4', '테스트출판사', 11000, true, '', '에세이', 2021, '지친 하루를 위로하는 힐링 에세이.'),
    (900005, '[TEST] 재미없는 책', '테스트작가5', '테스트출판사', 9900, true, '', '기타', 2020, '비선호 리뷰 테스트용 책.')
...
VALUES (9002, 900001, now() - interval '20 days');

INSERT INTO v_user_reviews (user_id, book_id, rating, created_at)
VALUES (9002, 900002, 4.5, now() - interval '15 days');

INSERT INTO v_user_library (user_id, book_id, added_at)
VALUES (9002, 900003, now() - interval '10 days');

-- ── 유저 9003 (C) — 비선호 리뷰 ──────────────────────────────────────────────
INSERT INTO v_user_reviews (user_id, book_id, rating, created_at)
VALUES (9003, 900005, 1.5, now() - interval '8 days');

-- ── 유저 9004 (D) — 담기만 ──────────────────────────────────────────────────
INSERT INTO v_user_library (user_id, book_id, added_at)
VALUES (9004, 900004, now() - interval '5 days');
