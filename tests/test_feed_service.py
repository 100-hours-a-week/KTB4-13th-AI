...
def test_프로필이_없으면_개인화를_끈_목록으로_간다(
    ...
    async def _cold(conn, req):
        return [{"book_id": 7, "match_score": 0}]
...
def test_취향_벡터를_못_만든_사용자도_같은_길로_간다(
    ...
    row = {"centroid": None, "tag_weights": "{}", "cold_start": True}
...
def test_프로필이_있으면_채점한_목록을_준다(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def _personalized(conn, req, centroid, tag_weights):
        assert centroid == [0.1, 0.2]
        assert tag_weights == {"에세이": 3}
        return [{"book_id": 9, "match_score": 84}]
...
def test_벡터_조회가_안_되면_규칙_점수만으로_답하고_알린다(
    ...
    row = {"centroid": "[0.1]", "tag_weights": "{}", "cold_start": False}

    async def _personalized(conn, req, centroid, tag_weights):
        raise RuntimeError("벡터 색인 이상")

    async def _cold(conn, req):
        return [{"book_id": 3, "match_score": 0}]
