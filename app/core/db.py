...
# 조회가 기대는 색인. 마이그레이션을 손으로 적용해서(위키 #160) 빠져도 에러가 나지 않고
# 표 전체를 훑어 느려지기만 한다. 그래서 기동할 때 확인한다(#154).
REQUIRED_INDEXES = (
    # ① 검색의 글자 조각 색인
...
async def missing_indexes(conn: asyncpg.Connection) -> list[str]:
    """REQUIRED_INDEXES 중 없거나 깨진(재색인 중) 색인 이름."""
...
async def report_missing_indexes(conn: asyncpg.Connection) -> None:
    """빠진 색인이 있으면 ERROR 로 남긴다. 서버는 그대로 뜬다.

    색인이 없어도 결과는 같고 느릴 뿐이라, 기동을 막으면 오히려 전체가 멈춘다.
    확인 자체가 실패해도 같은 이유로 로그만 남긴다.
    """
    try:
        missing = await missing_indexes(conn)
    except Exception:
        # 확인이 안 돼도 기동은 계속한다
        logger.exception("색인을 확인하지 못했습니다")
        return
    if missing:
        logger.error(
            "색인이 없거나 깨졌습니다: %s. 해당 조회가 표 전체를 훑어 느려집니다."
